export default function Carta({ carta, onClick }) {
	const chooseBackground = (carta) => {
		switch (carta.type) {
			case "ataque":
				return "bg-red-200";
			case "neutra":
				return "bg-yellow-200";
			case "defensa":
				return "bg-blue-200";
			default:
				return "bg-gray-200";
		}
	};

	return (
	
		//Cartas jugador 1

			<div className="carta" onClick={onClick}>
				<div className="rounded bg-white text-gray-800 hover:-translate-y-4 transition-all cursor-pointer">
					<div className=" text-lg align-center font-semibold py-1 capitalize ">
						{carta.titulo}
					</div>
					<div
						className={
							chooseBackground(carta) +
							" h-32 flex items-center justify-center py-12"
						}
					
					>

						<svg
							className="h-12 w-12"
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 512 512"
						>
							<path d="M177.1 228.6L207.9 320h96.5l29.62-91.38L256 172.1L177.1 228.6zM255.1 0C114.6 0 .0001 114.6 .0001 256S114.6 512 256 512s255.1-114.6 255.1-255.1S397.4 0 255.1 0zM416.6 360.9l-85.4-1.297l-25.15 81.59C290.1 445.5 273.4 448 256 448s-34.09-2.523-50.09-6.859L180.8 359.6l-85.4 1.297c-18.12-27.66-29.15-60.27-30.88-95.31L134.3 216.4L106.6 135.6c21.16-26.21 49.09-46.61 81.06-58.84L256 128l68.29-51.22c31.98 12.23 59.9 32.64 81.06 58.84L377.7 216.4l69.78 49.1C445.8 300.6 434.8 333.2 416.6 360.9z" />
						</svg>
					</div>
					<div className="p-4 text-sm">{carta.descr}</div>
				</div>
			</div>


			




		


		
	);
}
